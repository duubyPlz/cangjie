...
class BundleGraph {
  ...
  static fromAssetGraph(assetGraph, publicIdByAssetId = new Map(), assetPublicIds = new Set()) {
    ...
    for (let [, node] of assetGraph.nodes) {
      if (node.type === 'asset') {
        let asset = node.value; // Generate a new, short public id for this asset to use.
        // If one already exists, use it.

        let publicId = publicIdByAssetId.get(asset.id);

        if (publicId == null) {
          publicId = (0, _utils2.getPublicId)(asset.id, existing => assetPublicIds.has(existing));
  ...
  getContentHash(bundle) {
    ...
    this.traverseAssets(bundle, asset => {
      hash.update([asset.publicId, asset.outputHash, asset.filePath, asset.type, asset.uniqueKey].join(':'));
    });
